...
class Snake:
    ...
    def get_body_part(self, idx)-> SnakePart:
        """
        Gibt ein Körperteil der Snake zurück. 0 ist dabei der Kopf
        """
    ...
    def length(self):
        """
        Gibt die Länge der Snake zurück
        """
        return len(self.body)

    def has_full_length(self):
        """
        Zurück True zurück, wenn die Snake die vollständige Länge erreicht hat
        """
        return self.length() == self.max_length

    def move_head(self, move_direction, height, width):
        """
        ...
        next_head = self._get_next_head(move_direction, height, width)
    ...
    def _get_next_head(self, direction, height, width):
        head = self.get_head()

        moved_x, moved_y = DirectionUtil.move(head.x, head.y, direction, height, width)
