...
class DoubleDQNAgent(DQNAgent):
    ...
    def _compute_loss(
        ...
        # raise Exception("Hier Aufgabe 3 implementieren")
        ...
        # q_values_next_target[non_terminal_mask] = self.dqn.predict([state_tensor,
        #                                                             np.ones(shape=(state_tensor.shape[0],
        #                                                                                self.num_actions))])
        ...
        for i in range(batch_size):
            q_values_target[i, action_tensor[i]] = (
                reward_tensor[i] + self.hyper_parameters.gamma * q_values_next_target[i, q_values_next_argmax[i]]
            )

        actions_one_hot = np.eye(self.num_actions)[action_tensor]

        loss = self._fit(x=[state_tensor, actions_one_hot], y=q_values_target)
        self._update_target()
        return loss
